...
export default async function handler (req, res) {
  if (req.method === 'GET') {
    const [result] = await pool.query('SELECT id, nombre, descripcion, precio, id_categorias, cantidad, url, marca, precio - (precio * (descuento / 100)) AS descuento FROM productos WHERE cantidad > 0')
